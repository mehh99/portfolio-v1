...
describe("Terminal Component", () => {
  ...
  describe("Input Commands", () => {
    ...
    it("should display cmd history when user type 'history' cmd", async () => {
      await user.type(terminalInput, "command1{enter}");
      await user.type(terminalInput, "command2{enter}");
      await user.type(terminalInput, "command3{enter}");
      await user.type(terminalInput, "history{enter}");
    
      const typedCommands = screen.getByTestId("history").children;
      expect(typedCommands.length).toBe(3); // Harapkan 3 perintah di riwayat
    });
    
    it("should clear everything when user type 'cls' cmd", async () => {
      await user.type(terminalInput, "command1{enter}");
      await user.type(terminalInput, "cls{enter}");
    
      // Memastikan elemen output ada
      const output = screen.getByTestId("latest-output");
      expect(output).toBeInTheDocument(); // Pastikan output ada
    
      // Memastikan tidak ada output yang ditampilkan
      expect(output.children.length).toBe(0); // Harapkan tidak ada output setelah clear
    });
  ...
  describe("Redirect commands", () => {
    beforeEach(() => {
      // Mock window.open
      window.open = vi.fn(); // Menggunakan vi untuk mock window.open
    });
  
    afterEach(() => {
      // Reset mock untuk setiap pengujian
      vi.clearAllMocks();
    });
  
    it("should open the correct URL when the command is issued", async () => {
      // Simulasi perintah yang memicu window.open
      await user.type(terminalInput, "open https://example.com{enter}");
